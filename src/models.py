...
class BASEModel(nn.Module):
    ...
    def backbone(self, x_l, x_a, x_v):
        """
         ...
         """
        x_l = F.dropout(x_l.transpose(1, 2),
...
class CombModel(nn.Module):
    ...
    def backbone(self, x_l, x_a, x_v):
        ...
        proj_x_a = x_a if self.orig_d_a == self.d_a else self.proj_a(x_a)
        proj_x_v = x_v if self.orig_d_v == self.d_v else self.proj_v(x_v)
        # proj_x: [seq_len, batch_size, d]
        proj_x_a = proj_x_a.permute(2, 0, 1)
        proj_x_v = proj_x_v.permute(2, 0, 1)
        proj_x_l = proj_x_l.permute(2, 0, 1)

        if self.lonly:
            # trans_l_mem is a transformer encoder
            h_ls = self.trans_l_mem(proj_x_l)
            if type(h_ls) == tuple:
                h_ls = h_ls[0]
            # Take the last output for prediction
    ...
    def moe_nig(self, u1, la1, alpha1, beta1, u2, la2, alpha2, beta2):
        u = ((la1+2*alpha1) * u1 + u2 * (la2+2*alpha2)) / \
            ((la1+2*alpha1) + (la2+2*alpha2))
        la = la1 + la2
    ...
    def forward(self, x_l, x_a, x_v):
        ...
        # A residual block
        last_hs_proj_l = self.proj_l_2(F.dropout(
            F.relu(self.proj_l_1(last_h_l)), p=self.out_dropout, training=self.training))
        # last_hs_proj_l += last_h_l
        last_hs_proj_a = self.proj_a_2(F.dropout(
            F.relu(self.proj_a_1(last_h_a)), p=self.out_dropout, training=self.training))
        # last_hs_proj_a += last_h_a
        last_hs_proj_v = self.proj_v_2(F.dropout(
            F.relu(self.proj_v_1(last_h_v)), p=self.out_dropout, training=self.training))
        ...
        output, output_v, output_alpha, output_beta = self.split(
            output, output_v, output_alpha, output_beta)

        mu_l, v_l, alpha_l, beta_l = self.split(
            output_l_mu, output_l_v, output_l_alpha, output_l_beta)
        mu_a, v_a, alpha_a, beta_a = self.split(
            output_a_mu, output_a_v, output_a_alpha, output_a_beta)
        mu_v, v_v, alpha_v, beta_v = self.split(
            output_v_mu, output_v_v, output_v_alpha, output_v_beta)
...
class MOEModel(nn.Module):
    ...
    def backbone(self, x_l, x_a, x_v):
        """
         text, audio, and vision should have dimension [batch_size, seq_len, n_features]
         """
        # x_l, x_a, x_v: [batch_size, n_features, seq_len]
        x_l = F.dropout(x_l.transpose(1, 2),
                        p=self.embed_dropout, training=self.training)
        x_a = x_a.transpose(1, 2)
        x_v = x_v.transpose(1, 2)
        ...
        # proj_layer: conv1d(input_channel=ori_d, output_channel=d, kernel_size=1)
        # proj_x: [batch_size, d, seq_len]
        proj_x_l = x_l if self.orig_d_l == self.d_l else self.proj_l(x_l)
        proj_x_a = x_a if self.orig_d_a == self.d_a else self.proj_a(x_a)
        proj_x_v = x_v if self.orig_d_v == self.d_v else self.proj_v(x_v)
        ...
        # ???
        if self.lonly:
            # h_ls: [seq_len, batch_size, d]
            h_ls = self.trans_l_mem(proj_x_l)
            if type(h_ls) == tuple:
                h_ls = h_ls[0]
            # Take the last output for prediction
            # last_h_l = last_hs: [batch_size, d]
            last_h_l = last_hs = h_ls[-1]
    ...
    def split(self, mu, logv, logalpha, logbeta):
        # softplus: ln(1+exp(x))
        # actually output is mu, logv, logalpha, logbeta
        v = self.evidence(logv)
        alpha = self.evidence(logalpha) + 1
        beta = self.evidence(logbeta)
        return mu, v, alpha, beta
    ...
    def forward(self, x_l, x_a, x_v):
        ...
        # A residual block
        # 2 layer mlp(combined_dim -> combined_dim) 3 modalities
        last_hs_proj_l = self.proj_l_2(F.dropout(
            F.relu(self.proj_l_1(last_h_l)), p=self.out_dropout, training=self.training))
        # last_hs_proj_l += last_h_l
        last_hs_proj_a = self.proj_a_2(F.dropout(
        ...
        # linear (combined_dim -> output_dim)
        # 3 modalities(l, a, v) * 4 outputs(mu, v, alpha, beta) = 12
        output_l_mu = self.out_layer_l_mu(last_hs_proj_l)
        output_l_v = self.out_layer_l_v(last_hs_proj_l)
        output_l_alpha = self.out_layer_l_alpha(last_hs_proj_l)
        output_l_beta = self.out_layer_l_beta(last_hs_proj_l)
        ...
        # split: F.softplus(), mu doesn't change
        mu_l, v_l, alpha_l, beta_l = self.split(
            output_l_mu, output_l_v, output_l_alpha, output_l_beta)
        mu_a, v_a, alpha_a, beta_a = self.split(
            output_a_mu, output_a_v, output_a_alpha, output_a_beta)
